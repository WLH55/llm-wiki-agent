...
Document Revision 只执行一次标准解析与分块，并将完整结果持久化为 `content_chunks`。RAG 与 Wiki 都消费同一 Active Chunk Set，但分别作为独立的 `rag_index` 与 `wiki_generate` 子 Run 执行；RAG 负责 embedding、BM25/向量索引和 `knowledge_search`，Wiki 负责 Map/Reduce、页面、链接与证据。Wiki 页面本身永不写入 `content_chunks`，因此共享摄取不等于融合召回。
...
- Wiki 按 `chunk_index` 遍历全部 chunks，并将相邻 chunks 确定性组合成 token 预算内的 Map 批次。Redis 只保存 manifest、进度和 Map 结果，不复制 chunk 正文。
- 缺块、重复序号或顺序不一致会使 Wiki 任务显式失败；Wiki Worker 不得自行重解析原文件来掩盖共享摄取故障。
- 共享解析失败会同时阻塞 RAG 与 Wiki；解析成功后，embedding 失败与 Wiki 生成失败仍彼此隔离并可独立重试。
- Wiki-only 的 KB 也会产生 `content_chunks`，因此 chunk 正文仍进入物理 BM25 索引；在线 `knowledge_search` 是否可用继续由 RAG 配置控制。
- 旧 Active Revision 的 chunks 删除前，应用先清空证据表中匹配的 `content_chunk_id`；精确引文、位置和 Revision 引用继续保留。
