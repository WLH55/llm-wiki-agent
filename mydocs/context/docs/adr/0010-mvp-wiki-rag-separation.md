...
> 修订：2026-07-28 按 [ADR-0012](./0012-approved-rag-wiki-database-boundaries.md) 同步最终表边界与证据模型。
...
同时，Wiki 页面基于具体 Document Revision 的解析内容生成：长文在任务内临时分段、逐段提取后合并去重，但不依赖持久化 RAG chunks。若原文 chunk 与派生 Wiki 内容直接进入同一召回池，同一事实可能以两种表述重复占用 top-k；固定的 Wiki boost 还可能让二手转述压过一手原文，增加幻觉和溯源风险。
...
`content_chunks` 只保存当前生效 Document Revision 的 RAG 分块，不保留 `chunk_type`、`wiki_page_id` 或其他 Wiki 内容字段。Wiki 血缘由独立的 `wiki_page_document_refs` 与 `wiki_page_evidence_refs` 表表达；证据以具体 Revision、准确引文和原文位置成立，`rag_chunk_id` 只是 MVP 始终为空的未来快捷关联。
...
3. 利用 `wiki_page_evidence_refs` 的 Revision、准确引文和原文位置识别同源事实；如需映射 RAG Chunk，再启用预留的 `rag_chunk_id`。
...
- `content_chunks` 不为未经评测的融合承担预留字段成本；证据表只保留一个可空 `rag_chunk_id` 快捷关联，启用和回填必须另立 ADR。
